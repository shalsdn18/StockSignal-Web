...
- 모드: agent
- 관련 요구사항: REQ-F-004 (신호 이력 조회), REQ-F-012 (대시보드 통계 계산), REQ-U-001 (신호별 색상 구분), REQ-U-004 (네비게이션 고정 사이드바)  
- 프롬프트 목적: 메인 대시보드(dashboard.html)의 핵심 데이터 바인딩 구조를 보존하면서, 매수/매도 신호 상태에 따라 행(Row)의 색상이 다르게 표현되도록 동적 CSS 클래스 명세를 수립하고, 전체 사이트의 뼈대가 될 좌측 고정식 사이드바 UI 레이아웃을 정형화하기 위함. 
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: `REQ-F-013` (사용자 인증), `REQ-U-002` (반응형 레이아웃 대응)
- 프롬프트 목적: 플랫폼 보안 및 관제를 위한 계정 관리 전용 UI 폼 페이지(`login.html`, `register.html`)를 설계하고, 백엔드 JPA User 엔티티의 필드 구조에 호환되는 회원가입 입력 파라미터를 정의하는 동시에, 인증 화면 전용 그리드 속성 및 컴포넌트 스타일을 안전하게 통합하기 위함.
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: REQ-F-005 (종목별 검색), REQ-F-007 (신호 종류별 정렬), REQ-U-002 (반응형 레이아웃 대응)  
- 프롬프트 목적: 주식 관제 효율성을 극대화하기 위해 대시보드 화면에 종목 검색창과 신호 정렬용 드롭다운 상자를 결합하고, 사용자가 선택한 필터 상태가 초기화되지 않고 서버 파라미터와 연동되어 상시 유지되도록 설계하며, 모바일 기기(최대 너비 768px) 환경에서 컨트롤러 폼들이 유연하게 뷰포트를 채우도록 미디어 쿼리를 고도화하기 위함. 
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: `REQ-F-009` (브리핑 웹 페이지 노출), `REQ-U-004` (네비게이션 고정 사이드바)
- 프롬프트 목적: 모닝 브리핑 화면(`briefing.html`)의 마크업 프레임을 구축할 때 대시보드와 완전히 동일한 고정형 사이드바 레이아웃 구조를 상속받도록 처리하고, 백엔드로부터 인입될 데이터의 공백 상황에 대비한 타임리프 예외 분기 처리 및 AI가 생성한 줄바꿈·강조 HTML 태그가 화면에 깨짐 없이 온전히 출력되도록 동적 텍스트 언이스케이프(`th:utext`) 구조를 확립하기 위함.
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: REQ-F-014 (사용자 설정 관리), REQ-U-004 (네비게이션 고정 사이드바)
- 프롬프트 목적: 알림 연동 자산을 관리하는 시스템 설정 화면(settings.html)의 기본 구조를 전역 사이드바 레이아웃과 대칭 구조로 매핑하고, 사용자 엔티티 바인딩 시 데이터가 유실되어 발생할 수 있는 null 오류를 3항 연산자로 안전하게 방어하며, 서버의 설정 저장 처리 결과(성공/실패 플래시 메시지)가 사용자 화면 상단에 알림 위젯으로 정확히 표시되도록 가시성을 구현하기 위함
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: `REQ-NF-004` (확장 가능한 코드 구조 - 표준 MVC 준수)
- 프롬프트 목적: 백엔드의 복잡한 실제 데이터베이스 영속성 비즈니스 로직을 구축하기 전, 현재까지 완성된 타임리프 템플릿 뷰 자산(HTML/CSS)의 인터랙션과 데이터 매핑 상태를 로컬 톰캣 서버상에서 단독 수동 크로스 체크할 수 있도록 엔티티 가짜 데이터(Mock 데이터) 주입용 임시 UI 개발 테스트 컨트롤러 클래스(`TestUiController.java`)를 빌드하기 위함
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: REQ-NF-004 (확장 가능한 코드 구조 - 표준 MVC 준수)
- 프롬프트 목적: 완성된 임시 테스트용 웹 라우터 구조(TestUiController.java)의 기능을 확장하여, 단독 누락되었던 가입/로그인 엔드포인트를 마저 바인딩하고 특히 메인 루트 경로(GET "/") 호출 시 복수의 모의 주식 신호 개체 리스트(mockSignals)와 종합 통계 카운트 세트를 타임리프 대시보드 테이블에 주입하여 자바 컴파일러 에러 없는 통합 UI 검증 런타임을 구축하기 위함
- 지시사항:
  ```
...
- 모드: agent
- 관련 요구사항: `REQ-F-013` (사용자 인증 - 회원가입 프론트엔드 예외 방어)
- 프롬프트 목적: 백엔드 실제 서비스 레이어로부터 DTO 커맨드 객체를 전혀 위임받지 않는 독립 UI 테스트 환경 특성상, 회원가입 페이지 접속 시 타임리프 바인딩 전용 속성(`th:object`, `th:field`)으로 인해 빈번히 유발되던 템플릿 엔진 파싱 예외(HTTP 500 에러)를 해결하기 위해 해당 동적 명세를 표준 HTML 마크업 입력 폼 속성(`name`, `id`) 구조로 가볍게 전환하여 순수 브라우저 단독 렌더링 무결성을 100% 달성하기 위함
- 지시사항:
  ```text
...
- 모드: agent
- 관련 요구사항: REQ-F-004 (신호 이력 전체 조회), REQ-F-012 (대시보드 통계 계산), REQ-NF-004 (확장 가능한 코드 구조 - 표준 MVC 준수)
...
- 모드: agent
- 관련 요구사항: REQ-F-013 (사용자 인증)
- 프롬프트 목적: 시큐리티 세션 및 암호화 등 백엔드 코어 인프라 필터 레이어가 결합되기 전, 가입 서밋 완료 시 로그인 창으로 안전하게 분기하고 로그인 성공 시 서비스 대시보드로 수동 진입하는 가상 사용자 화면 시나리오 동선 및 페이지 리다이렉트 흐름을 모의 검증(Mock Test)하기 위함.
- 지시사항:
  ```
  현재 로그인(/login) 및 회원가입(/register) 화면에서 폼을 전송했을 때 다음 단계의 화면으로 넘어가지 않고 제자리에 머무는 리다이렉트 문제가 발생하고 있습니다. 
...
- 모드: agent
- 관련 요구사항: REQ-F-010 (투자 전략 메모 작성), REQ-F-011 (신호 데이터 삭제/편집), REQ-U-004 (네비게이션 고정 사이드바)
...
- 모드: agent
- 관련 요구사항: REQ-F-011 (신호 데이터 삭제/편집)
...
- 모드: agent
- 관련 요구사항: REQ-F-010 (투자 전략 메모 작성), REQ-NF-004 (확장 가능한 코드 구조 - 표준 MVC 준수)
...
- 모드: agent
- 관련 요구사항: REQ-F-004 (신호 이력 전체 조회 고도화), REQ-F-010 (투자 전략 메모 작성), REQ-NF-004 (확장 가능한 코드 구조 - 표준 MVC 준수)
- 프롬프트 목적: 데이터베이스에 저장된 1:N 관계형 투자 요약 메모(SignalMemo) 객체를 StockSignal 엔티티 내에 자바 8 양방향 @OneToMany 관계로 매핑하고, 타임리프 조건식 삼항 연산자를 활용해 대시보드 테이블 중앙에 가장 최근 메모 내역을 노출하되, 데이터가 부재할 시에는 하이픈(-) 기호가 정합성 있게 출력되도록 템플릿 뷰 출력을 최적화하기 위함.
- 지시사항:
```
...
- 지시사항:
  ```
@workspace
StockSignal-Web 프로젝트의 [REQ-F-012] 대시보드 통계 계산(승률 및 누적 손익 산출) 코어 알고리즘을 구현해줘.
종목별로 매수(BUY) 신호 이후 도달하는 매도(SELL) 신호를 매칭하여 실시간 통계를 산출하는 백엔드 로직이 필요해.

# 핵심 요구사항
1. DashboardStatisticsDto.java (신규 DTO)
- 패키지: com.stocksignal.dto
- 필드: long totalTrades(총 거래 횟수), long winningTrades(익절 횟수), double winRate(승률, 퍼센트), double cumulativeProfit(누적 손익)
- 생성자 및 Getter/Setter 작성. (승률은 winningTrades / totalTrades * 100 으로 내부 계산 혹은 서비스에서 계산)

2. StockSignalRepository.java
- 매매 시퀀스를 시간순으로 정확히 추적하기 위해 생성일 오름차순 전체 조회 메서드를 추가해줘.
- 메서드명: List<StockSignal> findAllByOrderByCreatedAtAsc();

3. StockSignalService.java
- 통계를 계산하는 메서드 `DashboardStatisticsDto calculateOverallStatistics()`를 구현해줘.
- [알고리즘 명세]
  A. repository.findAllByOrderByCreatedAtAsc() 호출.
  B. 데이터를 Ticker(종목명) 기준으로 그룹화 (예: Map<String, List<StockSignal>>).
  C. 각 Ticker별로 리스트를 순회하며 상태 머신 적용:
     - 상태 변수 `Double currentBuyPrice = null;` 선언.
     - 순회 중 SignalType이 BUY이면 `currentBuyPrice = signal.getPrice()`.
     - SignalType이 SELL이고 `currentBuyPrice != null` 이면 1회의 트레이드 완료로 간주:
       * 수익(PnL) = signal.getPrice() - currentBuyPrice;
       * totalTrades 1 증가.
       * 수익이 0보다 크면 winningTrades 1 증가.
       * cumulativeProfit에 PnL 누적.
       * 다음 거래 매칭을 위해 `currentBuyPrice = null;`로 초기화.
  D. 최종 누적된 값들을 바탕으로 DashboardStatisticsDto를 생성 및 반환. (Zero Division 방어 로직 포함)

4. DashboardController.java
- dashboard() GET 매핑 메서드 내부에서 `signalService.calculateOverallStatistics()`를 호출하도록 추가해줘.
- 반환받은 DTO 객체를 Model에 "statistics"라는 키로 담아 뷰 레이어에서 렌더링할 수 있도록 연결해줘.

# 제약 조건
- 메모리 누수나 N+1 문제가 발생하지 않도록 컬렉션 연산과 스트림(Stream) API를 효율적으로 사용할 것.
- 매수 신호가 연속으로 2번 오거나(물타기), 매수 없이 매도만 발생하는 엣지 케이스를 고려하여 `currentBuyPrice`의 null 체크 및 덮어쓰기 논리를 명확히(가장 최근 BUY 1건만 추적 등) 구성할 것.
  ```

### 날짜: 2026-06-14
- 모드: agent
- 관련 요구사항: REQ-F-014 (사용자 설정 관리), REQ-NF-004 (확장 가능한 코드 구조 - 표준 MVC 준수)
- 목적/상황: REQ-F-014(사용자 설정 관리) 구현을 위한 UserRepository 기반 데이터 수정(Update) 파이프라인 완결
- 사용한 프롬프트: 
```
@TestUiController.java
사용자가 설정 화면(settings.html)에서 텔레그램 Chat ID와 Bot Token을 입력하고 제출했을 때, 데이터베이스(H2) 유저 테이블에 영구적으로 수정 적재(Update)되도록 비즈니스 로직을 연결해줘.

1. UserRepository 의존성 주입:
   - com.stocksignal.repository.UserRepository를 TestUiController에 생성자 주입(DI)되도록 추가해줘.

2. settings() 메서드 (GET /settings) 수정:
   - 기존의 하드코딩된 new User() 생성 로직을 제거해줘.
   - 대신 userRepository.findByUsername("shalsdn18")로 DB에서 유저를 먼저 조회하도록 변경해줘.
   - 만약 DB에 해당 유저가 없다면, 기존의 기본값 정보를 가진 새 User 객체를 하나 만들어 userRepository.save()로 DB에 먼저 저장한 뒤 model에 담아 반환하도록 안전하게 설계해줘.

3. submitSettings() 메서드 (POST /test/settings-save) 수정:
   - 사용자가 폼으로 전송한 @RequestParam("telegramChatId") String chatId와 @RequestParam("telegramBotToken") String botToken을 파라미터로 받도록 수정해줘.
   - DB에서 "shalsdn18" 유저를 찾아와서 사용자가 입력한 두 가지 필드 값을 setter로 업데이트해 준 뒤, userRepository.save()를 호출해 영속성 테이블을 수정해줘.
   - 데이터 저장이 성공하면 화면에 알림을 띄울 수 있도록 redirect 경로에 RedirectAttributes를 활용해 successMessage("설정이 안전하게 저장되었습니다.")를 실어서 "redirect:/settings"로 리다이렉트해줘.
```
- 결과/메모: 테스트 계정 기준 텔레그램 연동 인프라 정보의 DB 영구 반영 및 UI 알림 스위칭 검증 마감.

---

- 모드: agent
- 관련 요구사항: REQ-F-012 (대시보드 통계 계산), REQ-NF-004 (확장 가능한 코드 구조)
- 프롬프트 목적: 대시보드 통계 연산 인프라 고도화 시, 핵심 알고리즘 엔진(SignalStatisticsService)의 유한 상태 기계(FSM) 매칭 타임라인이 유실되거나 꼬이지 않도록 원천 데이터를 과거 발생 순서(createdAt 오름차순)로 정렬하여 조회하고, 자바 스트림 API 필터링을 조율하여 집계된 종합 관제 지표 DTO 객체를 완벽히 빌드하기 위함.
- 지시사항:
```
@StockSignalService.java
대시보드 통계 계산(REQ-F-012) 고도화를 위해 calculateOverallStatistics() 메서드를 아래 규칙에 맞게 정밀하게 구현해줘. 

SignalStatisticsService 알고리즘은 데이터를 반드시 과거 발생 순서(createdAt 오름차순, oldest first)로 정렬해서 받아야 유한 상태 기계(FSM) 매칭이 정상 작동해. 만약 기존처럼 최신순(desc)으로 넘겨주면 매매 타임라인이 꼬여서 승률이랑 누적 수익이 다 망가지니까 주의해야 해.

1. 데이터 과거순 조회:
   - StockSignalRepository에서 모든 주식 신호를 과거순으로 가져오는 메서드(예: repository.findAllByOrderByCreatedAtAsc() 또는 그에 준하는 내림차순 반대 쿼리)를 호출해줘. 만약 레포지토리에 해당 메서드가 없다면 새로 선언해서 써줘.

2. 통계 엔진 가동 및 카운트 연산:
   - 과거순으로 정렬된 신호 리스트를 signalStatisticsService.calculate(signals)에 인자로 넘겨서 정밀 통계 결과(SignalStatistics)를 뽑아내줘.
   - 이와 별개로, 화면(dashboard.html) 사이드바와 위젯에 출력할 전체 BUY 신호 총 개수와 전체 SELL 신호 총 개수를 자바 스트림(Stream) 필터 카운트로 각각 계산해줘.

3. DTO 빌드 및 반환:
   - 위에서 계산한 모든 수치(totalCount, buyCount, sellCount, winRate, totalTrades, openPositions, cumulativeProfit)를 DashboardStatisticsDto 객체에 누락 없이 꽉 채워서 빌드한 뒤 반환해줘.
```

---

- 모드: agent
- 관련 요구사항: REQ-F-009 (브리핑 웹 페이지 노출), REQ-F-012 (대시보드 통계 계산), REQ-NF-004 (확장 가능한 코드 구조)
- 프롬프트 목적: 모닝 브리핑 화면(briefing.html)의 좌측 사이드바 통계 수치 패널이 메인 대시보드와 유기적으로 동기화되도록 기존의 하드코딩된 가짜 속성을 전면 제거하고, 백엔드의 StockSignalService 결합을 통해 도출된 진짜 실시간 통계 DTO 수치를 모델 속성에 매핑하여 시스템 데이터 정합성을 완결하기 위함.
- 지시사항:
```
@TestUiController.java
모닝 브리핑 화면(briefing.html)의 좌측 사이드바 통계 수치들이 가짜 데이터로 하드코딩되어 있는 문제를 해결해줘. 우리가 StockSignalService에 완성해 둔 진짜 통계 로직을 연결해서 실시간 수치로 채워지도록 고쳐줘.

1. StockSignalService 의존성 주입:
   - TestUiController 클래스 상단에 com.stocksignal.service.StockSignalService 필드를 private final로 추가해줘.
   - 기존 생성자 함수에 StockSignalService를 매개변수로 추가해서 의존성 주입(DI)이 정상적으로 이루어지도록 묶어줘.

2. briefing() 메서드 (GET /briefing) 수정:
   - 메서드 내부에서 signalService.calculateOverallStatistics()를 호출해서 진짜 대시보드 통계 DTO(DashboardStatisticsDto) 객체를 가져와줘.
   - 기존에 가짜 숫자로 하드코딩되어 있던 model.addAttribute("totalCount", 142) 같은 로직들을 싹 제거해줘.
   - 대신, 진짜 DTO 객체에서 값을 꺼내서 모델에 실어주도록 변경해줘.
     * 예시: model.addAttribute("totalCount", stats.getTotalCount());
```

---

- 모드: agent
- 관련 요구사항: REQ-F-015 (데이터 수집 모니터링), REQ-F-004 (신호 이력 전체 조회), REQ-NF-004 (확장 가능한 코드 구조)
- 프롬프트 목적: 외부 파이썬 봇이 신호를 송신했을 때 수집 엔진의 정상 가동 상태를 직관적으로 관제할 수 있도록 백엔드 서비스 레이어에 최종 웹훅 인입 시간을 기록하는 스펙을 확장하고, 타임리프 임시 예외 처리 및 포맷 연산자(${#temporals.format})를 주입하여 대시보드 상단에 실시간 관제 모니터링 알림창 위젯을 신설하기 위함.
- 지시사항:
```
@StockSignalService.java
외부 봇이 신호를 보냈을 때 최종 수신 시간을 기록하고 모니터링할 수 있도록 로직을 확장해줘.

1. 필드 추가:
   - 클래스 멤버 변수 자리에 최종 웹훅 수신 시간을 저장할 private LocalDateTime lastSignalReceivedAt; 필드를 추가해줘.

2. 수신 시간 업데이트:
   - 외부에서 신호가 생성되는 createSignal(StockSignalRequest request) 메서드가 실행될 때마다, 방금 추가한 lastSignalReceivedAt 필드에 현재 시간(LocalDateTime.now())이 저장되도록 업데이트 로직을 넣어줘.

3. Getter 메서드 추가:
   - 컨트롤러에서 이 타임스탬프 값을 읽어갈 수 있도록 public LocalDateTime getLastSignalReceivedAt() { return lastSignalReceivedAt; } 메서드를 추가해줘.

@DashboardController.java
대시보드 화면에 최종 신호 모니터링 타임스탬프를 출력할 수 있도록 모델 데이터를 추가해줘.

- dashboard() 메서드 (GET / 및 GET /dashboard 주소) 내부에서 signalService.getLastSignalReceivedAt()를 호출해서 최종 수신 시간 값을 가져와줘.
- 가져온 값을 model.addAttribute("lastSignalReceivedAt", 수신시간변수)라는 이름으로 모델에 실어서 화면단으로 넘겨주도록 코드를 추가해줘.

@dashboard.html
대시보드 메인 화면 상단에 외부 데이터 수집 상태를 실시간으로 관제할 수 있는 모니터링 알림창(위젯)을 추가해줘.

1. 위치 배정:
   - <main class="container dashboard-main"> 태그가 시작되고 바로 직후, 즉 Summary cards section이 나오기 바로 직전에 새로운 모니터링 <section> 또는 <div> 블록을 개설해줘.

2. 위젯 UI 및 타임리프 바인딩 요구사항:
   - 클래스명은 기존의 수려한 테마와 어우러지도록 api-box 또는 card 스타일을 활용해서 깔끔하게 잡아줘.
   - 내부에는 초록색 동그라미 기호(🟢)와 함께 "시스템 관제: 데이터 수집 엔진 가동 중"이라는 안내 문구를 넣어줘.
   - 그 바로 옆에 "최근 신호 웹훅 수신: [시간]" 형태의 문구를 출력해줘.
...
- 목적/상황: [REQ-NF-003] 시스템 로그 관리
- 사용한 프롬프트:
  ```
 @workspace
StockSignal-Web 프로젝트의 [REQ-NF-003] 시스템 로그 관리 체계를 SLF4J와 Logback을 활용하여 구축해줘. 외부 API 장애 및 DB 커넥션 유실 시 신속한 원인 파악이 가능해야 해.

# 핵심 요구사항
1. logback-spring.xml (신규 생성)
- `src/main/resources/` 경로에 파일을 생성해 줘.
- Console Appender를 구성하고, File Appender는 TimeBasedRollingPolicy(일별 롤링) 및 SizeBasedTriggeringPolicy(크기 제한)를 적용해 줘.
- 에러 트래킹을 위해 로그 레벨이 ERROR인 것만 별도의 파일(`error-%d{yyyy-MM-dd}.log`)로 격리하여 저장하는 Appender를 추가해 줘.
- 로그 포맷 패턴: `%d{yyyy-MM-dd HH:mm:ss.SSS} [%thread] %-5level %logger{36} - %msg%n`

2. GlobalExceptionHandler.java (신규 생성)
- `com.stocksignal.exception` 패키지에 `@ControllerAdvice` (또는 `@RestControllerAdvice`) 컴포넌트를 만들어 줘.
- DB 커넥션/쿼리 예외(`org.springframework.dao.DataAccessException`) 및 외부 API 호출 예외(`org.springframework.web.client.RestClientException`)를 `@ExceptionHandler`로 포착해 줘.
- 포착 시 SLF4J `log.error("Exception message", e)` 형식으로 전체 스택 트레이스를 남기도록 구성해 줘.

3. 기존 서비스 로깅 보강
- `StockSignalService.java`와 `TelegramNotificationService.java`에 Lombok `@Slf4j` 어노테이션을 추가해 줘.
- 주요 비즈니스 예외 구간(API 토큰 누락 시 우회 로직, Retry 실패 후 Recover 메서드 진입점 등)에 `log.warn()` 및 `log.error()`를 적재적소에 배치해 줘.

# 제약 조건
- Java 코어의 `System.out.println()`이나 `e.printStackTrace()` 사용을 절대 금지하고, 모든 출력은 SLF4J Logger로 일원화할 것.
...
### 날짜: 2026-06-16
- 목적/상황: 웹훅 신호 전송 시 데이터베이스 저장 및 텔레그램 발송은 정상 처리되나, 클라이언트 응답 단계에서 본문이 텅 빈 채 HTTP 500 (Internal Server Error) 예외가 반환되는 현상 해결
- 사용한 프롬프트:
  ```
  웹훅 신호를 보내면 대시보드에 신호가 뜨고 텔레그램 알림도 오는데, PowerShell 창에는 500 내부 서버 오류가 발생합니다. curl.exe -i로 확인해 보니 응답 본문이 비어 있고(Content-Length: 0), 서버 로그에는 'HttpMessageNotWritableException: No converter for [class java.util.HashMap]' 에러가 찍혀 있습니다. 원인을 찾아서 해결해줘.
- 결과/메모: pom.xml 파일 내 jackson-databind 의존성에 잘못 지정되어 있던 <scope>test</scope> 범위를 제거. 테스트 환경에만 격리되어 있던 JSON 변환기를 실제 런타임 환경에 정상 가동하도록 복구하여 정상적인 결과 응답(JSON 형식) 플러시 완료.

---

- 목적/상황: pom.xml 설정을 수정한 직후 애플리케이션 재구동 시 ClassNotFoundException: com.stocksignal.StockSignalApplication 메인 클래스 로드 실패 에러 대응
- 사용한 프롬프트:
  ```
  pom.xml 수정 후 Java를 다시 실행하니까 'Error: Could not find or load main class com.stocksignal.StockSignalApplication' 오류가 발생하면서 서버가 켜지지 않습니다. 해결책을 알려줘.
- 결과/메모: IDE(VS Code) 내부 컴파일러 캐시와 메이븐 의존성 동기화가 깨진 상태를 우회하기 위해, 터미널에서 mvn clean compile 명령어로 빌드 공간을 초기화 및 컴파일 재빌드 마감. 이어서 mvn spring-boot:run 파이프라인을 구축하여 스프링 부트 서버의 정상 구동 확인 완료.

---

- 목적/상황: 웹훅 연동 성공 후 스마트폰으로 전달되는 텔레그램 알림 메시지가 줄바꿈 및 특수기호가 정상 파싱되지 않고 퍼센트 범벅(%F0%9F%93%88...)의 URL 인코딩 형태로 깨지는 가독성 이슈 해결
- 사용한 프롬프트:
  ```
  웹훅 전송 후 텔레그램 메시지는 잘 도착하는데 알림창에 '%F0%9F%93%88 Stock Signal Alert%0D%0ATicker%3A TSLA...' 형태로 보기 어렵게 깨져서 옵니다. 정상적인 문자열로 오도록 고쳐줘.
- 결과/메모: TelegramNotificationService에서 기존에 적용되어 있던 URLEncoder.encode() 수동 변환 로직을 제거. RestTemplate의 고유 가이드라인인 가변 인자 주소 플레이스홀더 템플릿(?chat_id={chatId}&text={text}) 구조로 전환하여 '중복 인코딩(Double Encoding)' 트랩을 방지하고 UI 가독성 100% 정상화 완료.
